...
export default function Evm(props: IProps) {
  ...
  const changeBaseFee = (e: ChangeEvent<HTMLInputElement>) => {
...

        {
          loading ? null : (
            feeList?.map((item, index) => {
              const fee = formatUnits(item.fee, 9)
              return (
                <Button
                  key={item.title}
...
function CustomFee(props: {
  ...
  changeBaseFee: (e: ChangeEvent<HTMLInputElement>) => void,
  changePriorityFee: (e: ChangeEvent<HTMLInputElement>) => void
}) {
  const { t } = useContext(I18nContext)!
  const { baseFee, priorityFee, changeBaseFee, changePriorityFee } = props

  const displayBaseFee = formatUnits(baseFee, 9)
  const displayPriorityFee = formatUnits(priorityFee, 9)

  return (
    <div className={css.custom}>
      <div>
        <label className={css.customFeelabel}>{t('component.fee.label.basefee')}</label>
        <Input
          value={baseFee}
          onChange={changeBaseFee}
          suffix={<span className={css.customFeeTip}>wei = {displayBaseFee} Gwei</span>}
        />
      </div>
      <div>
        ...
        <Input
          ...
          suffix={<span className={css.customFeeTip}>wei = {displayPriorityFee} Gwei</span>}
        />
      </div>
    </div>
  )
